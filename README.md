...
My EFI doesn't require `FakePCIID.kext` to get the Intel I225 2.5 Gigabit Ethernet Controller working. It also doesn't require any wake-related SSDTs like `SSDT-GPRW.aml` since there are no wake issues. `SSDT-USBW.aml` – which doesn't do anything besides turning the screen back on with one instead of two mouse/keyboard inputs - is also unnecessary. This is without a doubt the most sophisticated Z490 Vision G EFI you can find on Github to date! And just for fun, I added Clover, too.
